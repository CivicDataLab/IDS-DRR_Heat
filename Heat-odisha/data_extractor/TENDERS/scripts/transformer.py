...
# ---------------------------------------------------------------------------
# Configuration -- adjust these paths/names to match your repo layout.
# ---------------------------------------------------------------------------
BASE = os.getcwd()
# The flood source lives in Sources/TENDERS; set this to your heat source dir.
DATA_PATH = os.path.join(BASE, 'Heat-odisha', 'data_extractor', 'TENDERS', 'data')
SHAPEFILE = os.path.join(BASE, 'Heat-odisha', 'Maps',
                         'od_ids-drr_shapefiles', 'odisha_block_final.geojson')
GEOTAGGED_CSV = os.path.join(DATA_PATH, 'heattenders_blockgeotagged.csv')
 
# Merge keys (tender side -> shapefile side)
TENDER_DISTRICT_COL, TENDER_BLOCK_COL = 'DISTRICT_FINALISED', 'BLOCK_FINALISED'
SHAPE_DISTRICT_COL, SHAPE_BLOCK_COL = 'dtname', 'block_name'
 
# Build indicators only from strong-signal heat tenders when that information
# is available (heat_signal == 'strong', written by the patched classifier).
...
def build_indicators(df):
    ...
    # 1. Total awarded value per block per month
...
def main():
    ...
    # Normalize case before merging: BLOCK_FINALISED is title-case (e.g.
    # "Chitrakonda") while the shapefile's block_name is upper-case (e.g.
    # "CHITRAKONDA"), so a direct merge matches nothing.
    heat_df['_merge_district'] = heat_df[TENDER_DISTRICT_COL].astype(str).str.upper().str.strip()
    heat_df['_merge_block'] = heat_df[TENDER_BLOCK_COL].astype(str).str.upper().str.strip()
    od_gdf['_merge_district'] = od_gdf[SHAPE_DISTRICT_COL].astype(str).str.upper().str.strip()
    od_gdf['_merge_block'] = od_gdf[SHAPE_BLOCK_COL].astype(str).str.upper().str.strip()

    heat_df = heat_df.merge(
        od_gdf,
        on=['_merge_district', '_merge_block'],
        how='left',
    )
