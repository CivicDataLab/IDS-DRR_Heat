...
Heat-tender counterpart of geocode_rc.py (which geotags Assam's FLOOD
tenders down to revenue circle). Same village/block/revenue-circle matching
logic, but reads the district-tagged heat tenders (heattenders_districtgeotagged.csv,
written by geocode_district_heat.py) and writes heattenders_RCgeotagged.csv --
the revenue-circle-level analogue of Odisha's heattenders_blockgeotagged.csv.
"""
...
ASSAM_RCS = gpd.read_file(root + r'/data_extractor/Maps/Geojson/assam_rc_2024-11.geojson', driver='GeoJSON')

RC_HQs = list(ASSAM_RCS[ASSAM_RCS.HQ=='y']['revenue_ci'])

tenders_df = pd.read_csv(root + '/data_extractor/TENDERS/data/heattenders_districtgeotagged.csv', keep_default_na=False)
...
for FOCUS_DISTRICT in tqdm(ASSAM_VILLAGES.district_2.unique()):
    ...
    for idx, row in tenders_df_FOCUSDISTRICT.iterrows():
        ...
        tenders_df_FOCUSDISTRICT.loc[idx,'tender_revenueci'] = tender_revenueci
        tenders_df_FOCUSDISTRICT.loc[idx,'tender_revenueci_location'] = tender_revenueci_location
...
MASTER_DF.to_csv(root+'/data_extractor/TENDERS/data/heattenders_RCgeotagged.csv', index=False)
print('Wrote', root+'/data_extractor/TENDERS/data/heattenders_RCgeotagged.csv')
